...
CPU_ANTUTU = {
    ...
    "Apple A16": 1400000,
    "Apple A15": 1200000,
    "Bionic A19 Pro": 2900000,
    "Bionic A18 Pro": 2500000,
    "Bionic A18": 2300000,
    "Bionic A17 Pro": 1600000,
    "Bionic A16": 1400000,
    "Bionic A15": 1200000,
    "Tensor G2": 800000,
    "Tensor": 600000,
    "Dimensity 8500 Ultra": 1400000,
    "Dimensity 8500 Extreme": 1400000,
    "Dimensity 8450": 1300000,
    "Snapdragon 4s Gen 2": 420000,
...
def parse_charging_time(wattage: int, battery_mah: int) -> int:
    w = max(5, wattage)
...
SHEET_HEADERS = [
    ...
    "display_refresh_hz", "build_quality_score", "charging_mins"
]
...
def push_to_sheet(client, rows):
    ...
    if ws.row_count > 1:
        ws.batch_clear([f"A2:{col_letter}{ws.row_count}"])
    values = [[row.get(h, "") for h in SHEET_HEADERS] for row in rows]
    if values:
        ws.update(f"A2:{col_letter}{1 + len(values)}", values)
